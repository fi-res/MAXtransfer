...
from pymax.types import FileAttach, PhotoAttach, StickerAttach, VideoAttach
...
if USE_SOCKET_CLIENT:
    client = SocketMaxClient(
        MAX_PHONE, token=MAX_TOKEN, work_dir="data/cache", reconnect=True
    )
else:
    client = MaxClient(
        MAX_PHONE, token=MAX_TOKEN, work_dir="data/cache", reconnect=True
...
async def tg_retry[T](
    func: Callable[..., CoroutineType[Any, Any, T]] | Callable[..., T],
    *args,
    retries: int = 10,
    **kwargs
) -> T:
...
async def process_max_message(message: Message) -> None:
    ...
    if message.link and message.link.type == "FORWARD":
        text = (
            f"<i>-> Переслано от {await get_sender_name(message.link.message.sender)}</i>\n"
            + text
    ...
    for attach in message.attaches or []:
        if isinstance(attach, PhotoAttach):
            photos.append(
                BufferedInputFile(
                    (await download_content(attach.base_url)).getvalue(), "photo.jpg"
                )
            )
        elif isinstance(attach, VideoAttach):
            video_data = await client.get_video_by_id(
                MAX_CHAT_ID, message.id, attach.video_id
            )
            if video_data:
                video = BufferedInputFile(
                    (await download_content(video_data.url)).getvalue(), "video.mp4"
                )
        elif isinstance(attach, FileAttach):
            file_data = await client.get_file_by_id(
                MAX_CHAT_ID, message.id, attach.file_id
            )
            if file_data:
                file = BufferedInputFile(
                    (await download_content(file_data.url)).getvalue(), attach.name
                )
            ...
            if attach.lottie_url:
                sticker = BufferedInputFile(
                    compress((await download_content(attach.lottie_url)).getvalue()),
                    "sticker.tgs"
                )
            else:
                photos.append(
                    BufferedInputFile(
                        (await download_content(attach.url)).getvalue(), "sticker.png"
                    )
                )
            text += "<i>Стикер</i>"
        else:
    ...
    if len(photos) == 1:
        ...
        tg_message = (
            await tg_retry(
                bot.send_media_group, TG_CHAT_ID, media, reply_to_message_id=reply
            )
        )[0]
    elif video:
        tg_message = await tg_retry(
            bot.send_video, TG_CHAT_ID, video, caption=text, reply_to_message_id=reply
        )
    elif file:
        tg_message = await tg_retry(
            bot.send_document, TG_CHAT_ID, file, caption=text, reply_to_message_id=reply
        )
    elif sticker:
        await bot.send_message(TG_CHAT_ID, text)
        tg_message = await tg_retry(
            bot.send_sticker, TG_CHAT_ID, sticker, reply_to_message_id=reply
        )
    else:
        tg_message = await tg_retry(
            bot.send_message, TG_CHAT_ID, text, reply_to_message_id=reply
        )

    l.info(
        "receive message text=%s photos=%s video=%s file=%s",
        text.replace("\n", "  "),
        len(photos),
        bool(video),
        bool(file)
    ...
    if chat.new_messages > 0:
        messages = (
            await client.fetch_history(
                chat.id, int(time_since_epoch() * 1000), 0, chat.new_messages
            )
            or []
        )
...
async def send_handler(message: types.Message):
    ...
    try:
        if ADMIN_USER_ID and message.from_user.id != ADMIN_USER_ID:
            await tg_retry(
                message.reply, "Отправка сообщений доступна только администратору"
            )
            return
        ...
        if message.reply_to_message:
            reply = next(
                (
                    ...
                    if tg_id == message.reply_to_message.message_id
                ),
                None
            )
        ...
        sent_msg = await client.send_message(
            text, MAX_CHAT_ID, attachment=photo, reply_to=reply
        )
...
async def main():
    ...
    done, _ = await wait([tg_task, max_task, stop_task], return_when=FIRST_COMPLETED)
    for task in done:
